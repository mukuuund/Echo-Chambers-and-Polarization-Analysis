...
# 3. APPLY VADER SENTIMENT  (FIXED: avoid fake 0 sentiment)
...
for n, data in G.nodes(data=True):
    ...
    # FIX: treat empty / super short tweets as missing sentiment (None)
    if len(text.split()) < 3:
        score = None
    else:
...
print(f"[ ] Sentiment computed (including None) for {len(node_sentiment)} nodes")
...
for i, comm in enumerate(communities):
    ...
    # -------- Sentiment stats (FIXED: ignore None, use NaN if not enough data) --------
    sentiments = [
        G.nodes[n].get("sentiment")
        for n in comm
        if G.nodes[n].get("sentiment") is not None
    ]
    ...
    if len(vectors) >= 2:
        ...
        sem_mean = np.nan       # FIX: mark as missing, not 0
...
# 8. DETERMINE ECHO CHAMBERS (FIXED: ignore NaNs, require real polarity)
 
# Start with no echoes
df_stats["is_echo"] = False

# Use only communities with valid sentiment variance + semantic cohesion
valid = df_stats.dropna(subset=["sentiment_variance", "semantic_cohesion", "mean_sentiment"])

if len(valid) > 0:
    sent_var_thr = np.percentile(valid["sentiment_variance"], 25)
    sem_coh_thr = np.percentile(valid["semantic_cohesion"], 75)

    # FIX: enforce a minimum semantic cohesion, to avoid "all-zero" issues
    sem_coh_thr = max(sem_coh_thr, 0.25)

    # FIX: require non-neutral sentiment (abs(mean) >= 0.05)
    echo_mask = (
        (df_stats["sentiment_variance"] <= sent_var_thr) &
        (df_stats["semantic_cohesion"] >= sem_coh_thr) &
        (df_stats["mean_sentiment"].abs() >= 0.05)
    )

    df_stats.loc[echo_mask, "is_echo"] = True
else:
    print("[!] No valid communities with both sentiment and semantic cohesion. No echoes flagged.")
